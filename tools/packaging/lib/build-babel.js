...
export function buildWithWebpack(destination, componentName, componentEntry, ignoredFiles) {
    return exec(`webpack --config webpack-transform.config.js --componententry="${componentEntry}" --componentname=${componentName} --distpath="${destination}" --ignoredFiles=${ignoredFiles} --bail`);
}
